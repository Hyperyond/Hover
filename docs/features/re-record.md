...
- **One spec per command.** v0.11 doesn't ship `re-record --all` or `--failed` — both rejected on purpose ([see FAQ](/faq#why-no-re-record-all-or-failed)). For now: re-record specs one at a time.
...
See the [FAQ entry](/faq#my-ui-changed-and-my-saved-spec-breaks-what-now) for the full reasoning.
