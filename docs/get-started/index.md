...
- [Pick an agent](./agents) — six agents wired today: Claude (hard sandbox) plus Codex, cursor-agent, aider, gemini-cli, and qwen-code (soft sandbox). One-file registry to add more.
...
- **One artefact, multiple formats.** Save the same session as a Playwright spec, a Hover Skill (replayable later by name), or a Jira / Xray-compatible test case CSV — and with the optional [`@hover-dev/security`](/features/security) plugin loaded, a Security spec entry also appears in the Save dropdown.
