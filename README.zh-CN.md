...
- **v0.3.x** —— **`@hover-dev/next` —— Next.js 16+ Turbopack 原生集成** ✓ **（你在这里）**。三件套 —— `withHover(nextConfig, opts)` 包 `next.config.mjs`、`<HoverScript />` Server Component 放进 `app/layout.tsx`、`register()` helper 放进 `instrumentation.ts`。现有 `webpack-plugin-hover` 只覆盖 `next dev --webpack`；这个包是 Turbopack 原生路径。`npx @hover-dev/cli add` 会自动把 Next 项目路由到这里。
