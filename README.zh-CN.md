...
- **🛡️ Save as Security spec（v0.12 新增）。** 装 `@hover-dev/security` 之后，agent 的 `replay_flow` MCP 工具多了两个参数 —— `intent`（"IDOR：访问别人的订单"）和 `expectStatus`（如 403）。两个一起传，Hover **把这次 replay 记成 security check**。一轮探测之后，Result 卡的 Save-as 菜单多出第四项：**Security spec**。它落盘 `__vibe_tests__/<slug>.security.spec.ts` —— 纯 `@playwright/test`，用 `request` fixture，每条 check 一个 `test()`，4xx 期望还自动加一条 PII 泄漏粗检查。CI 跑它不需要 MITM、不需要 agent。今早发现的 IDOR，从此每个 PR 都自动回归。详见下文 [Security spec](#security-spec)。
...
`cursor-agent`、`aider`、`gemini-cli`、`qwen-code` 都已经接入并可在下拉切换（都是软沙箱，⚠ 标）。再加新的（或你自己的 coding-agent CLI）只需要在 [`packages/core/src/agents/registry.ts`](./packages/core/src/agents/registry.ts) 加一个文件。
...
如果你想用的 agent 不在已支持列表里（目前是 `claude` / `codex` / `cursor-agent` / `aider` / `gemini-cli` / `qwen-code`），去 [`packages/core/src/agents/registry.ts`](./packages/core/src/agents/registry.ts) 加一行就行 —— 欢迎 PR。
...
- **v0.4.x** —— **点击元素 → 生成精准修复提示词。** ✓ 独立 footer Fix 按钮 + 元素 picker + 意图弹窗 + clipboard 传递。Vite transform 在每个 host JSX 元素上打 `data-hover-source="file:line:col"`（兼容 React 19 —— 用 `enforce: 'pre'` 跑在 `@vitejs/plugin-react` 把 JSX 折叠之前）。React 组件链来自 `_debugOwner`。JSX / Vue / Svelte / Astro 全栈源码归因在 v0.8.x 落地。
...
- **v0.11.x** —— **Spec 弹性：⟳ Re-record + Saved sessions overlay + FAQ** ✓。UI 变化大到 saved spec 跑红时，不用手改 `.spec.ts` —— 点 ⟳ Re-record。Agent 读 spec JSDoc 里的 `Original prompt:`，在*当前* UI 上重放，Hover 用新 selector 覆盖文件。两种触发：widget 新的 **📜 Saved sessions** overlay（Skills + Specs 两个 tab），或者终端 `pnpm hover re-record <spec>`。CI 本身保持纯 Playwright —— AI 只在*写测试*那步介入，不在*跑测试*。README + 文档站新加顶级 FAQ 章节解释这套弹性模型 vs Stagehand/Midscene 的 CI 自愈方案。
- **v0.12.x** —— **Security spec 录制语义** ✓。`@hover-dev/security` 激活时，agent 的 `replay_flow` MCP 工具新增 `intent` + `expectStatus` 参数。两个一起传，Hover **把这次 replay 记成 security check**。每条 check 累积在控制平面；widget 的 Save-as 菜单多出 **Security spec** 入口，把 check 列表落盘为 `__vibe_tests__/<slug>.security.spec.ts` —— 纯 `@playwright/test` 用 `request` fixture，每个 check 一个 `test()`，4xx 期望自动加 PII 泄漏粗检查。CI 跑它不需要 MITM、不需要 agent。同时新增：服务端 `HoverPluginManifest` 加 `saveHandlers` API + widget `WidgetPluginSpec` 加 `saveEntries` 字段 —— 插件现在能注册完全自定义的 save 流程，不动 core 的 spec / skill / csv pipeline。闭合了 v0.7 打开的 security testing loop。
- **v0.13.x** —— **录制 / 回放对齐** ✓ **（你在这里）**。三件相关的改动改善 saved spec 在真实条件下的表现。**(1) 可见性前置（visibility prelude）** —— spec emit 现在把每个交互步骤包成 `{ const el = …; await expect(el).toBeVisible(); await el.<action>; }`，不再是裸 `.click()`。UI 漂移（按钮被塞进关着的 `<details>` / kebab 菜单）现在在 ~3 秒报 `Locator expected to be visible`，不再等到 30 秒泛型 actionability 超时 —— 同一个 case，类别更清晰、更快。**(2) Record 模式把 `page.goto` 当成第一步落地** —— Record 启动时抓 `window.location.href` 作合成步，spec 回放时正确打开页面再点（之前裸 emit 从 `about:blank` 起跑，看着像 Hover bug，其实是缺一个起手 `goto`）。**(3) "Reload before recording" 设置项**（可选，默认关）—— 开启后点 Record 会先弹 confirm 确认 reload，让录制起点和回放起点完全一致，给需要严格对齐的用户。同时新增 `?drift=on` 可见性漂移 lab（在 `examples/basic-app`），以及 9 条覆盖 prelude 的 vitest case。
- **v0.14.x 或独立仓库** —— 规划中 —— **Chrome 扩展。** 脱离 bundler 插件依赖，让 Hover 能驱动**任意** tab（staging URL、第三方站点）。可能在独立仓库（`hover-extension`）里维护而不放进 monorepo —— Web Store 上传是手动流程，扩展的发版节奏不应该被 monorepo PR 锁着。代价是没有源码归因（没 transform 跑过），换取通用页面覆盖。

v0.13.x 是你今天能用的。

## 项目状态

🟢 **v0.13.0 已发布。** 全部五个宿主 bundler 集成都 dogfood 可用：Vite、Astro、Nuxt、Next.js (Turbopack)、webpack 5 —— 外加 React Native Web（跑在 Vite 上的示例）。v0.13 闭合了录制 / 回放的对齐缺口：spec emit 把每个交互包进 visibility prelude，UI 漂移时不再等 30 秒 actionability 超时，而是 ~3 秒报 `Locator expected to be visible`；Record 模式终于在第一步合成 `page.goto`；可选的"Reload before recording"设置让需要严格对齐的用户能把录制起点和回放起点彻底拉齐。前面的 arc：v0.12（Security spec 录制语义 —— `replay_flow` 加 `intent` + `expectStatus`、Save-as 加 Security spec 入口、服务端 `saveHandlers` + widget `saveEntries` 插件扩展点）、v0.11（Spec 弹性 —— ⟳ Re-record + Saved sessions overlay + FAQ）、v0.10（多 tab agent 健壮性 + `aider` / `gemini-cli` / `qwen-code`）、v0.9（widget 插件 UI 贡献协议 + `@hover-dev/security` 迁移）、v0.8（多框架源码归因 + Next 插件支持）、v0.7（Security testing 插件 API + `@hover-dev/security` MITM 代理 / 抓包面板）、v0.6（Voice mode —— push-to-talk STT + 进度朗读，纯浏览器原生 Web Speech API）、v0.5（Record + Exists / Says / Equals sub-toolbar）、v0.4（点击 → Fix prompt + Vite 源码归因）、v0.3（Next.js Turbopack 原生集成）。
