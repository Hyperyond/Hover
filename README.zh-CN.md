...
**自带 CLI —— 订阅 *或* API key 都行。** Hover 直接 spawn 你 `PATH` 上已有的 coding-agent CLI（`claude` / `codex` / …）。可以骑你已经在付费的订阅，也可以把自己的模型 API key 填进 widget（它被透传到 CLI 的环境变量，只存在你的浏览器里，绝不上传）。无论哪种，LLM 成本都是 authoring 时的一次性开销 —— 不会变成每次绿构建都要交的税，因为存下来的 `.spec.ts` 用 `npx playwright test` 永远跑得起来，回路里没有 agent。

## 为什么是 Hover

这个领域已有不少好工具；Hover 是当你为另一个维度 —— **产物可移植性** —— 做优化时的产物。
...
Hover 并不想做更好的*运行时* AI runtime。它让存下来的产物就是纯 `@playwright/test` 代码、零 AI 依赖：agent 的活干到 "save" 为止，CI 是纯 Playwright —— **零 token、CI 里不接任何 key**。

### 三种固化方式

done 卡片上一个 **💾 Save as ▾** 把同一次验证过的会话固化成三种形态 —— 每个文件都进 git，没装 Hover 的同事也能读。

| | `📜 .spec.ts` | `💾 SKILL.md` | `📋 .case.csv` |
|---|---|---|---|
| **落在** | `__vibe_tests__/` | `.claude/skills/` | `__vibe_tests__/` |
| **谁读** | Node + Playwright（CI） | Claude Code / agent | Xray · Zephyr · Jira |
| **受众** | CI、开发 | 未来探索的你 | QA · PM |
| **确定性** | 硬契约 | 尽力回放 | 人工审核 |

<p align="center">
  <img src="docs/screenshots/05-save-dropdown.png" alt="Save 下拉 —— Playwright spec、Claude Code Skill、Jira test case (CSV)" width="48%" />
...
- **抗变 + bug 发现** —— 可见性守卫的选择器会大声失败、而不是卡 30 秒超时；**⟳ Re-record** 用 spec 的原始 prompt 重跑、重写选择器;agent 的 `## Findings` 落进按严重度分级的卡片。
- **Record 模式、语音模式、⌖ Fix prompt、安全测试** —— 手动走一遍流程得到同样的 spec；按住说话的语音 prompt（中文 / English）；点任意元素 → 一段带源码归因的 fix prompt 进剪贴板；可选的 MITM 代理[安全插件](https://gethover.dev/docs/features/security)。详见[文档站](https://gethover.dev/docs)。
...
```bash
npx @hover-dev/cli add        # 探测打包器 + 包管理器，自动改配置（幂等；--dry-run 预览）
```
...
**已并入 `main`（下个版本发布）：** 结构化 spec 输出 —— Page Object + fixture、`test.step` 分块、sidecar、popup 配对、默认关闭的 AI 优化环节（含 `// KNOWN BUG` 标注）、`.hover/rules/` 种子库、按 prompt 收放的探索范围。
...
[**`nexu-io/open-design`**](https://github.com/nexu-io/open-design)（**Local CLI Agent First** 架构）、[**Playwright Codegen**](https://playwright.dev/docs/codegen)（确定性 spec 即产物）、[**Stagehand**](https://github.com/browserbase/stagehand) / [**Midscene**](https://github.com/web-infra-dev/midscene)（证明了 LLM 能驱动真实浏览器）。Hover 缩短回路：authoring 时驱动一次，然后退场。
