...
**已并入 `main`（下个版本发布）：** 结构化 spec 输出 —— Page Object + fixture、`test.step` 分块、sidecar、popup 配对、默认关闭的 AI 优化环节（含 `// KNOWN BUG` 标注）、`.hover/rules/` 种子库、按 prompt 收放的探索范围。还有 **`codeContext`** —— 一个可选、只读、带围栏的源码读取器（`read_source` MCP；secrets / `.env` / `.git` / 构建产物都排除），让**红色渗透模式变白盒**：agent 拿真实的 query / 鉴权代码确认漏洞、报告直接指到 `文件:行`，写测试时也能用真实代码里的选择器。默认关 —— 不开就还是纯浏览器。
