...
AI 优化环节和安全模式都靠**种子**教 —— 小的「示例 / 探针配方」。**全套目录随 Hover 内置**（不用克隆第二个仓库，也不用 `curl`）；往 `<root>/.hover/rules/` 丢你自己的 JSON 就能加新模式，或在 `<root>/.hover/seeds.json` 里按名字把某个内置项列进 `disabled` 来关掉它（不用 fork、不用写插件代码）。当前内置：
...
| `file-upload` | 隐藏的 `<input type=file>` → `waitForEvent('filechooser')` + `setFiles` |
| `dialog` | 原生 alert/confirm/prompt → 在点击前注册 `page.once('dialog', …)` |
| `network-gated-assertion` | 触发 XHR 的点击 → 断言前先 `waitForResponse` |
| `oauth-popup` | 通过打开新标签页的第三方弹窗登录 |

（普通 popup / 新标签页配对也在翻译器里硬编码了；`oauth-popup` 种子额外教优化环节完整的第三方登录形态。）

**安全探针种子** —— 🟠 安全 / 🔴 渗透 模式会试的(8 个访问控制 + 9 个漏洞):
...
| `cors-reflected-origin` | CORS 配置错误 | 🔴 vuln |
| `jwt-claim-tamper` | JWT 篡改 | 🔴 vuln |
