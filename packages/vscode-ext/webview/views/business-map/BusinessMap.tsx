...
function nodeStyle(n: MapNode, selected: boolean): React.CSSProperties {
  const base: React.CSSProperties = {
    ...
    width: 190,
    boxShadow: selected ? "0 0 0 2px var(--vscode-focusBorder)" : undefined,
  };
...
function label(n: MapNode): string {
  if (n.kind === "line") {
    ...
    return mark + n.label + (n.route ? `  ${n.route}` : "");
  }
  return n.label;
}
...
function filterGraph(graph: Graph, query: string, uncoveredOnly: boolean): Graph {
  ...
  return { app: graph.app, nodes, edges, stats: { lines, covered, areas } };
}
