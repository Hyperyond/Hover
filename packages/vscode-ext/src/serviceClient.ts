...
export interface ServiceClientPool {
  ...
  run(text: string, sessionId?: string, accounts?: RunAccount[], env?: { id?: string; name?: string }, sourceAccess?: 'always' | 'ask' | 'deny', enginePort?: number, isolateContext?: boolean, intensity?: string): boolean;
...
export function connectServicePool(handlers: PoolHandlers): ServiceClientPool {
  ...
  return {
    ...
    run(text: string, sessionId?: string, accounts?: RunAccount[], env?: { id?: string; name?: string }, sourceAccess?: 'always' | 'ask' | 'deny', enginePort?: number, isolateContext?: boolean, intensity?: string): boolean {
      ...
      if (!ws) return false;
      ws.send(JSON.stringify({ type: 'command', payload: { text, sessionId, accounts, env, sourceAccess, isolateContext, intensity } }));
      return true;
    },
    sendSourceApproval(approvalId: string, allow: boolean, enginePort?: number): void {
      const ws = target(enginePort);
