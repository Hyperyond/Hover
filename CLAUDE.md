...
- `packages/probe-engine` is the private shared probe engine consumed by `@hover-dev/security` and `@hover-dev/pentest`.
- `packages/security` (`@hover-dev/security`) and `packages/pentest` (`@hover-dev/pentest`) are the security / pentest mode plugins, staged into the extension's engine. (The old bundler-plugin / in-page-widget packages — `vite-plugin-hover`, `@hover-dev/astro` / `nuxt` / `next`, `webpack-plugin-hover`, `@hover-dev/cli`, `@hover-dev/widget-bootstrap`, `@hover-dev/transform-source` — have been removed; their per-bundler integration notes survive only in git history.)
...
- `<scope>` is the package or sub-area: `core`, `vscode-ext`, `security`, `pentest`, `probe-engine`, `example`, `agents`, `playwright`, `mcp`, `ci`, `deps`.
