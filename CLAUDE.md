...
The **`hover-dev` VSCode extension (`packages/vscode-ext`) is the going-forward distribution.** The npm bundler-plugin path — `vite-plugin-hover`, `@hover-dev/astro` / `nuxt` / `next`, `webpack-plugin-hover`, `@hover-dev/cli`, `@hover-dev/widget-bootstrap`, plus the in-page widget — is **frozen**: no new versions are planned, existing published versions stay installable. Don't invest in those packages.

**`@hover-dev/core` is NOT frozen** — it stays the engine, consumed by the extension as *local source* (the extension's `stage-engine` step `npm pack`s the local `packages/core`, never the registry), so core keeps evolving without being (re)published to npm. **Do not delete `core`; do not delete the plugin/widget packages yet** — sunset cleanup (code removal, `npm deprecate`) is a separate, later task, deliberately deferred until the extension actually ships on the Marketplace (deprecating the only currently-usable install path before its replacement exists would strand users). The README is likewise left as-is until then.
...
(The bundler-dogfood examples — `astro-app` / `nuxt-app` / `next-app` / `webpack-app` / `rn-web-app` — were removed when the bundler-plugin path was frozen; the remaining examples are the testing-surface targets the extension drives.) Each remaining example still installs `vite-plugin-hover` for the legacy in-page widget; with the VS Code extension you don't need it — the extension drives any dev server over CDP regardless. A widget-bearing example boots its own service on `127.0.0.1:51789` (auto-bumps to 51790+ when several run at once); `payment-provider` has no service at all.
...
Modes in the extension: **both 🟠 security and 🔴 pentest are wired.** `scripts/stage-engine.mjs` packs `@hover-dev/security` AND `@hover-dev/pentest` into `engine/node_modules`, and `host.mjs` loads both into `startService({ plugins })` — `@hover-dev/security` (default export → mode `security`) and `@hover-dev/pentest/plugin` (the `./plugin` subpath → mode `pentest`; the package's main entry is a report library, not a manifest). pentest reuses security's resident MITM + control plane via the exported `startSecurityRuntime()` (refcounted, shared); the two modes are mutually exclusive (`conflictsWith: ['security']`). security crystallizes confirmed authz findings into `.security.spec.ts`; pentest is offensive (origin-locked, in-band) and writes a Markdown findings report. **Runtime caveat:** security/pentest pull `mockttp` + `@peculiar/asn1-*` + `get-port`; under the extension's bundled Electron-Node the known ERR_REQUIRE_ESM (get-port) / asn1-schema-collision issues (see FAQ) could surface — plugin load is fail-soft (logs + degrades to "mode unavailable", never blocks engine boot), so verify on a real sideload.
...
**Exceptions: `@hover-dev/core` and `@hover-dev/widget-bootstrap`** point `main` / `exports` at `dist/*.js` (and ship a `dev: tsc --watch`). Reason now: the VS Code extension's staged engine resolves `@hover-dev/core/dist/service.js` (`host.mjs` imports `@hover-dev/core/service`), so core must build to `dist`. (Historically this also let the frozen `@hover-dev/next` `await import(...)` core under Turbopack — vercel/next.js#82945.) A root `postinstall` builds both after every `pnpm install`, so fresh clones have usable `dist/`. Editing `core/src/` during extension dev needs a `pnpm --filter @hover-dev/core build` (or a `dev` watch terminal) so the staged engine picks it up; `pnpm --filter hover-dev package` re-packs core fresh into the `.vsix`.
