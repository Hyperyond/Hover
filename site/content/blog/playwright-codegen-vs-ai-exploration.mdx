...
description: "Playwright's built-in recorder is free, local, and writes clean code, but it only transcribes clicks. Where codegen is the right tool, and where authoring from intent changes the job."
date: 2026-05-22
author: "Hover"
...
Recording your clicks and authoring a test are different jobs.
...
Codegen has no idea what you're trying to accomplish. You can't tell it "test the login flow" and walk away; you perform the login flow yourself, click by click. It adds an assertion when you click the toolbar to record one. It can't tell which of your clicks mattered and which were you hunting for a menu. The raw output is a draft you then edit.

That isn't a gap in the tool. A stenographer writes down what was said. Codegen writes down what you did.
...
Three things change as a result. You author by saying what you want instead of performing it, which helps when the flow is long or when you're specifying a test for an app someone else built. The agent infers assertions from the goal, so you don't have to remember to record the check that proves it worked. And the agent walks past a cookie banner or an interstitial that would interrupt a manual recording.

Hover's output is the same kind of file codegen produces: a plain `@playwright/test` spec with semantic locators that runs in CI with no model. Hover spends the model while authoring and hands you ordinary Playwright. You don't trade codegen's clean, dependency-free output for a vendor format.
