...
The catch has always been that someone has to write and maintain those tests, and on a vibe-coded app the person who should write them is the person who didn't read the code. That's where [Hover](/) comes in. It's a free, open-source MCP server you add to the coding agent you already run. The agent drives your real app once, in your real browser, and Hover crystallizes that run into a plain `@playwright/test` spec with semantic `getByRole` and `getByLabel` selectors. No new test framework, no proprietary format. The file is standard Playwright you can read and edit.
