...
description: "An AI optimizing for 'make it work' writes the happy path, not the authorization check. IDOR and broken access control are the holes your click-through tests never touch. Point your agent at Hover and it finds them from your dev browser."
date: 2026-06-16
author: "Hover"
...
## Replay the real calls with mutated IDs

Hover is a free, open-source MCP server you add to the coding agent you already run, and access-control testing is built for exactly this. It runs a local HTTPS man-in-the-middle proxy in front of your dev browser, so the agent sees the real API calls your app makes, headers and bodies and all.
...
## Go offensive on your own origin

Access-control testing probes one control at a time. An offensive sweep goes further. It is origin-locked and own-app-only, so it only ever attacks the dev target you point it at. Within that boundary it probes for SQL injection, XSS, SSTI, SSRF, and IDOR, the way an attacker would, against the running app rather than a static read of the code.

The sweep writes a Markdown findings report: what it tried, what responded, and where the app gave something away. You read it like a security review, then fix what is real.

Both of these run through the same agent you already use to author tests. There is no separate tool to install, no bundler plugin, no second account. You go from authoring tests to probing access control to running an offensive sweep against the same dev browser. (More on the loop is [here](/blog/security-and-pentest-in-the-same-editor-chat/).)
