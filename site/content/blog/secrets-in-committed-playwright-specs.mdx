...
This is the unglamorous half of the [explore-then-crystallize](/blog/ai-authored-playwright-tests/) idea. An AI authoring tests is only useful if the artifact it leaves behind is one you'd actually commit. A spec with a live token in it is not that artifact, so the sanitizer runs on the way out, every time.
