...
Runs get slower. A model that looks at a screenshot and decides what to click takes longer than a selector resolving against the DOM. Vision models take longer.
...
No agent lives in that file. It runs with `npx playwright test`, in your CI, with zero tokens and no call to any provider. The model did its work at the one moment intelligence helped, figuring out the flow. What it left behind is ordinary Playwright.
