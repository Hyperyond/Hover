...
(The original surface that surfaced this was an in-page widget injected via bundler plugins — now the frozen path. The current way to use Hover is the VS Code extension, which does the same CDP detection from the editor.)

Hover knows your dev-server origin, and the service knows which Chrome it can reach over CDP, from `/json/list`. Compare the two and Hover answers a question you'd otherwise have to puzzle out: is a debug Chrome actually up and on your app?

Three answers, three states. Same window means the origins match and the agent can drive that tab. Wrong window means a debug Chrome exists but isn't on your app; one click brings the right tab forward. No debug Chrome means none is running; one click launches it on your dev URL. You never reason about ports or protocols. Hover walks you to a working state.
