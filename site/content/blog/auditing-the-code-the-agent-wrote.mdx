...
That's the same discipline Hover sells. The [optimize pass](/docs/features/optimization-pass/) re-reads a saved spec instead of trusting the first draft. [Security and pentest modes](/blog/pentest-mode-attack-your-own-app/) attack the running app instead of trusting that a green happy path means the code is sound. Running an audit over our own source was the same move, aimed at ourselves. If we wouldn't ship Hover's output without a verification step, we shouldn't ship Hover without one either.
