...
The **VS Code extension** (`hover-dev`) is the going-forward distribution; it is built, engine-staged, and packaged separately with `pnpm --filter hover-dev package` (→ a `.vsix`) and published to the Marketplace under the `hyperyond` publisher — not part of the npm tag-push workflow below.
