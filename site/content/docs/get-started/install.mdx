...
One command wires Hover into your agent (Claude Code shown — any MCP-capable agent works):
...
Any MCP-capable coding agent can use Hover. Add `@hover-dev/mcp` as an MCP server in your agent's configuration (for Claude Code, the one-liner above; for others, point the server command at `npx -y @hover-dev/mcp`). The tools and the `/mcp__hover__test_app` flow are identical across agents.
