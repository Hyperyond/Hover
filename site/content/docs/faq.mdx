...
3. CI now runs your API-test spec with the same effective auth as Hover recorded (the crystallized file is `<slug>.api-test.spec.ts`).
...
No. Hover spawns the coding-agent CLI on your local `PATH` (`claude`, `codex`, `gemini`, `qwen`, etc.) and that CLI talks to its own provider (Anthropic, OpenAI, Google, or your own self-hosted endpoint). `@hover-dev/core` itself has no LLM SDK code, no telemetry, no upload path. The Node service binds to `127.0.0.1` only and refuses connections from any other interface.
