...
- **Re-record it.** Open the **Specs** view and click **⟳ Re-record**. The agent reads the spec's JSDoc `Original prompt:` header ("log in then add a todo") and replays it against the *current* UI, then overwrites the `.spec.ts` with new selectors. About 30 seconds, about $0.10 per spec. Review the `git diff` before committing. See the [Re-record a spec](/docs/features/re-record) page for the full walkthrough.
