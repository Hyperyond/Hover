...
One command adds the MCP to your agent (Claude Code shown — any MCP-capable agent works):
...
`.hover/` is Hover's [living test wiki](/docs/features/living-wiki/): a business map (`hover-map.md`), remembered business rules (`memory/`), per-spec sidecars (`sidecars/`), and disposable working data (`cache/`). The intended policy in your project is to ignore `cache/` and commit the rest — it's plain Markdown and JSON that versions alongside the code it describes.
