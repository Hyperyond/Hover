...
The Fix prompt is a feature of the original in-page widget (the [frozen bundler-plugin path](/docs/get-started/install#frozen-bundler-plugin-path)). It relies on the widget's `data-hover-source` stamping and is **not** part of the VS Code extension chat. The **[VS Code extension](/docs/get-started/install)** is the recommended way to use Hover.
