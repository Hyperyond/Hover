...
Hover covers API and application-security testing inside **QA Testing** mode, as two capability toggles you turn on (they are not standalone modes — the mode picker is just **Flow** + **QA Testing**):
...
- 🔴 **Penetration testing** — offensive, white-box testing against your **own** dev app: SQLi / XSS / SSTI / SSRF / open-redirect / IDOR. Because it is destructive it runs as a **separate second pass** after the functional verify pass, and it is **off by default** (enabling it asks for confirmation). The output is a findings report that says what it did *and didn't* test. Authorized own-app testing only.
...
Both capabilities run inside the VS Code extension: `@hover-dev/api-test` and `@hover-dev/pentest` are packed into the extension's staged engine, so flipping a QA toggle is all it takes — no separate install.

## Why a gated capability

Hover's **Flow** mode is for *building features*, and **QA Testing** explores for functional defects. Security testing is for *attacking what you just built* — the agent's prompt is different ("look for authz bypass", not "test the happy path"), its captured traffic is routed through a local MITM proxy, and the chat **tints orange** for API (red for penetration) so you can never forget you're in altered state. Penetration testing is destructive, so it is gated behind an explicit toggle + confirmation and runs as its own pass.
...
1. In the Hover chat, switch to **🟢 QA Testing** and turn on the **🟠 API testing** toggle.
...
5. When findings show up in the Result + Findings cards, click **Save as spec** to crystallise the recorded `replay_flow` checks into a `__vibe_tests__/<slug>.api-test.spec.ts` regression test that runs in CI with vanilla `@playwright/test`. See [Save as an API-test spec](/docs/features/api-test-spec/).
