...
Hover covers application-security testing in the **same chat** as functional testing — flip a mode switch. There are two security modes:

- 🟠 **Security** — business / authorization testing. Hover routes the debug Chrome through a local HTTPS MITM and lets the agent re-issue captured API calls with mutations to probe for IDOR / authentication bypass / parameter tampering. Confirmed findings crystallize into `.security.spec.ts` regression specs that run in CI without the proxy.
...
Both are taught by **seeds** — small probe recipes (8 access-control classes + 9 vulnerability classes). The catalogue ships built-in; add your own under `<root>/.hover/rules/`.

Both modes run inside the VS Code extension: `@hover-dev/security` and `@hover-dev/pentest` are packed into the extension's staged engine, so flipping the mode switch is all it takes — no separate install.
...
The proxy is **resident**: it starts with the engine (transparent passthrough by default) and the single debug Chrome is launched pointed through it. There is no second browser — entering Security mode just flips the proxy from passthrough to recording.
...
1. In the Hover chat, flip the **mode switch** to **🟠 Security**.
...
5. When findings show up in the Result + Findings cards, click **Save as → Security spec** to crystallise the recorded `replay_flow` checks into a `__vibe_tests__/<slug>.security.spec.ts` regression test that runs in CI with vanilla `@playwright/test`. See [Save as Security spec](/docs/features/security-spec).
...
In **🟠 Security** mode (the MITM-replay business/authz mode described above), the agent will refuse to attempt:
...
For contributors who want to extend the plugin or write a similar one (`@hover-dev/security` is built on the same plugin API as any optional Hover mode):
