...
`type` is one of `business-rule` (default), `expected-behavior`, `validation`, or `access-policy`. The fact is written under `.hover/memory/` — see [your app's test knowledge](/docs/features/living-wiki/).
