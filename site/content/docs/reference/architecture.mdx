...
```
chat UI (VS Code extension webview)   [or: frozen in-page widget Shadow DOM]
  ↕ WebSocket (127.0.0.1)
...
- The **service** is allowed to spawn one specific Chrome: the isolated debug Chrome under `<tmpdir>/hover-chrome`. The extension launches it on demand when you start a run from the chat (on the frozen bundler-plugin path it could also launch at dev-server startup with `autoLaunchChrome: true`). It is *not* the user's primary Chrome profile.
...
- The frozen in-page widget lives in a **Shadow DOM** marked with `data-hover="true"` so Playwright tests can skip it. (The VS Code extension renders its chat in a webview, outside the page entirely, so there's nothing in the page for Playwright to filter.)
- The engine binds to **`127.0.0.1` only**, with no telemetry and no upload path. The frozen bundler plugins are no-op in production builds.
