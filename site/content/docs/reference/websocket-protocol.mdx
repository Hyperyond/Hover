...
The client (the extension chat, or the frozen in-page widget) and the engine (`@hover-dev/core`) communicate over a single WebSocket bound to `127.0.0.1`. Every message is `{ type, payload }`.
