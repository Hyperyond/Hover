...
The `hover` bin (`@hover-dev/cli`) is a **living secondary surface** for authoring specs from the terminal — handy when you'd rather not (or can't) use the editor chat. The [VS Code extension](/docs/get-started/install) is the recommended way to use Hover; `hover run` complements it for terminal workflows.
...
|---|---|
| `run "<prompt>"` | **Drive the debug Chrome from the terminal.** `--save <slug>` crystallizes a spec. See [hover run](#hover-run-cli-mode). |
| `optimize <spec>` | The optional [AI optimization pass](/docs/features/optimization-pass) — propose an improved spec (candidate + diff, original kept). |
...
| `setup` | **Frozen — bundler-plugin wirer.** Installs + wires the in-page widget into your bundler. See [`setup` (frozen)](#setup-frozen-bundler-plugin-wirer). |

`run` is terminal-native authoring; `optimize` / `extract` / `re-record` post-process specs you've already saved — the same operations the extension's Optimize / Re-record do. `setup` belongs to the frozen bundler-plugin path.
...
## `setup` (frozen bundler-plugin wirer)

<Callout type="warning" title="Frozen — bundler-plugin path">

`hover setup` installs and wires the original in-page widget into your bundler config. It is part of the [frozen bundler-plugin path](/docs/get-started/install#frozen-bundler-plugin-path) — still works, but no longer the recommended way to use Hover. For the going-forward surface, install the [VS Code extension](/docs/get-started/install).

</Callout>

`setup` reads your `package.json` to pick the right Hover integration (Vite / Astro / Nuxt / Next / Webpack), sniffs your lockfile to pick the package manager (pnpm / yarn / bun / npm), spawns the install command, then uses [magicast](https://github.com/unjs/magicast) to AST-mutate your bundler config.
