...
- **[Wiki lint](/docs/features/mcp-tools/#the-mcp-prompts).** `/mcp__hover__lint` health-checks `.hover/`: it cross-checks the business map against your spec files and run ledger for deleted spec refs, regressed coverage, and unmapped specs, then layers judged checks (contradictory rules, code routes missing from the map) and offers fixes.

## What's next

- **A deeper test-knowledge graph.** A richer `.hover/` memory layer — a relationship graph linking flows, pages, and the endpoints they touch — over the Business Map, remembered rules, and the wiki lint that ship today. No source uploaded.
