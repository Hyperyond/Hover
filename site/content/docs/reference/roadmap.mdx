...
| **VS Code extension (`hover-dev`)** | **Going-forward distribution** — chat in the editor (no in-page widget), engine bundled inside the extension, Specs / Sessions / Environments views, `@account` multi-environment credentials, 🟠 Security + 🔴 Pentest modes, optional AI optimize pass, Re-record, PR-CI workflow generation. The npm bundler-plugin path is now frozen. | ✅ Shipped (Marketplace) |
