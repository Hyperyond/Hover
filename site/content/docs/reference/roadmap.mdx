...
| **Structured spec output** (on `main`) | Page Objects + fixtures lifted from repeated flows (`hover extract`), named `test.step` stages, `.hover/<slug>.json` sidecars, `Promise.all` popup/new-tab pairing, the off-by-default **AI optimization pass** (diff-reviewed candidate, original always kept, `// KNOWN BUG` flagging), the `.hover/rules/` **seed library**, prompt-scoped exploration, and an optional model **API key** passed to the spawned CLI's env | ✅ Shipped (**you are here**) |
