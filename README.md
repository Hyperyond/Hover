...
- **v0.10.x** — planned — **Recording semantics for security mode.** Re-purpose the Record button while a security mode (or future probing mode) is active so a session captures the agent's replay decisions + asserts the server response shape, crystallising into a security regression spec.

v0.8.x is what you can use today.

## Project status

🟢 **v0.8.0 shipped.** Dogfood-ready across all six host bundlers: Vite, Astro, Nuxt, Next.js (Turbopack), webpack 5, and React Native Web. v0.8 generalises the v0.4 JSX source-attribution stamp to Vue / Svelte / Astro via a new private `@hover-dev/transform-source` package that gets inlined into each of the 5 integration shims (so consumers install only the shim they need), and gives `@hover-dev/next` first-class plugin support via `register()`'s second argument. Earlier arcs: v0.7 (Security testing plugin API + `@hover-dev/security` MITM proxy / captured-flow inspector), v0.6 (Voice mode — push-to-talk STT + spoken step narration, browser-native Web Speech API), v0.5 (Record + Exists / Says / Equals sub-toolbar), v0.4 (Click → Fix prompt + Vite source-attribution transform), v0.3 (Next.js Turbopack-native integration).
