...
- **v0.3.x** — **`@hover-dev/next` — Next.js 16+ Turbopack-native integration.** The existing `webpack-plugin-hover` only covers `next dev --webpack`; Next 16 ships Turbopack as the default bundler and Turbopack does not load webpack plugins. A native Next module closes the largest framework gap in Hover's bundler-coverage story.
