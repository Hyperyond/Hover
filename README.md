...
- **v0.10.x** — planned — **multi-tab / cross-origin polish + more agents + Chrome extension.**
  ...
  - More agents wired into the [registry](./packages/core/src/agents/registry.ts) — `aider` / `gemini-cli` / `qwen-code`.
...
v0.9.x is what you can use today.

## Project status

🟢 **v0.9.0 shipped.** Dogfood-ready across all six host bundlers: Vite, Astro, Nuxt, Next.js (Turbopack), webpack 5, and React Native Web. v0.9 introduces the widget plugin-UI contribution protocol — plugins can now own their entire widget surface, and `@hover-dev/security` migrates onto it (the v0.7 hardcoded `client.js` branches are gone). `cursor-agent` joins `claude` + `codex` in the agent registry. Earlier arcs: v0.8 (multi-framework source attribution + Next plugin support), v0.7 (Security testing plugin API + `@hover-dev/security` MITM proxy / captured-flow inspector), v0.6 (Voice mode — push-to-talk STT + spoken step narration, browser-native Web Speech API), v0.5 (Record + Exists / Says / Equals sub-toolbar), v0.4 (Click → Fix prompt + Vite source-attribution transform), v0.3 (Next.js Turbopack-native integration).
